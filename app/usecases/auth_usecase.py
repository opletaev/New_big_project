...
class AuthUsecase:
    ...
    async def login_user(
        ...
        user = await UserService(UserRepository).get_user_info_by_factory_employee_id(  # Тут дерьмо какое-то
        # НАСТРОИТЬ ЗАВИСИМОСТИ
            body.factory_employee_id
            )
        if not user or not self.service.verify_password(body.password, user.hashed_password):
